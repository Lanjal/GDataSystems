...
from .uploadsedcor_dialog import UploadSedcorDialog
import os.path
...
class UploadSedcor:
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'UploadSedcor_{}.qm'.format(locale))
    ...
    def tr(self, message):
        return QCoreApplication.translate('UploadSedcor', message)
    ...
    def initGui(self):
        icon_path = ':/plugins/uploadsedcor/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'&GDS-Upload Pontos Sedcor'),
            callback=self.run,
    ...
    def unload(self):
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&GDS-Upload Pontos Sedcor'),
                action)
    ...
    def run(self):
        ...
        if result:
            ...
            con_string = "dbname='"+db+"' host='"+host+"' port='5432' user='"+user+"' password='"+pwd+"' key=id type=POINT table=\"public\".\"ponto\" (geom)"
            ...
            cur = conn.cursor()
            cur.execute("DELETE FROM sedcor")
            conn.commit()
            cur.execute("SELECT UpdateGeometrySRID('sedcor', 'geom', Find_SRID('public', 'ponto', 'geom'))")
            conn.commit()
            cur.execute("INSERT INTO sedcor(id,geom,projeto,alvo,ponto,utme,utmn,elev,coletado,who) SELECT id,geom,projeto,alvo,ponto,utme,utmn,elev,coletado::boolean,who FROM ponto")
            conn.commit()
